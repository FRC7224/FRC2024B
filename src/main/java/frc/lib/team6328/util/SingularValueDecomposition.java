 ...
 *
 * <p>For an m-by-n matrix A with m >= n, the singular value decomposition is an m-by-n orthogonal
 * matrix U, an n-by-n diagonal matrix S, and an n-by-n orthogonal matrix V so that A = U*S*V'.
 *
 * <p>The singular values, sigma[k] = S[k][k], are ordered so that sigma[0] >= sigma[1] >= ... >=
 * sigma[n-1].
 *
 * <p>The singular value decompostion always exists, so the constructor will never fail. The matrix
